...
export async function up(): Promise<void> {
  ...
  try {
    ...
    if (matchResultExists.rows.length === 0) {
      await client.query(`
        ALTER TABLE matches 
        ADD COLUMN player1_match_result VARCHAR(20),
        ADD COLUMN player2_match_result VARCHAR(20),
        ADD CONSTRAINT player1_match_result_check 
          CHECK (player1_match_result IN ('WIN', 'LOSS', 'DRAW', 'NO_MATCH')),
        ADD CONSTRAINT player2_match_result_check 
          CHECK (player2_match_result IN ('WIN', 'LOSS', 'DRAW', 'NO_MATCH'))
      `);
    ...
    if (payoutStateExists.rows.length === 0) {
      await client.query(`
        ...
        ADD CONSTRAINT player2_payout_state_check 
          CHECK (player2_payout_state IN ('NOT_PAID', 'PAID'))
      `);
